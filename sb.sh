...
# ==========================================
# Sing-box 6-in-1 極致穩定架構版
# 特性：純 ASCII 終端，繁體中文，全協議默認，獨立 Argo
# ==========================================
...
cyber_progress() {
    local task="$1"; local total_steps=20
    ...
    echo -e "\n ${GREEN}[ SYS.OK   ] 指令執行完畢。${NC}"
}

matrix_reveal() {
    local secret_data="$1"; local length=${#secret_data}
    [ $length -gt 80 ] && length=80 # 防止終端換行錯亂
...
glitch_error() {
    local msg="$1"
    for i in {1..4}; do
        echo -ne "\r ${BG_RED}[ SYS.CRIT ] ${BOLD}${msg}${NC} "
        sleep 0.1
        echo -ne "\r ${RED}[ SYS.CRIT ] ${BOLD}${msg}${NC} "
        sleep 0.1
    done
    echo ""
}
...
apply_cert() {
    ...
    else msg_warn "埠 80 資源衝突或不可達，放棄 Standalone 模式。"; fi
    ...
    if [ "$standalone_success" = false ]; then
        ...
        cyber_progress "向 Cloudflare 下發 API 驗證請求..."
...
install_warp() {
    ...
    if ! command -v warp-cli >/dev/null 2>&1; then
        cyber_progress "掛載 Cloudflare WARP 節點模塊..."
...
install_fast() {
    print_logo
    boot_sequence
    typewriter " ${CYAN}[ SYS.INIT ]${NC} ${YELLOW}開始執行全協議自動部署腳本...${NC}"
    ...
    cyber_progress "向內存注入架構配置並喚醒守護進程..."
...
install_custom() {
    print_logo
    boot_sequence
    typewriter " ${CYAN}[ SYS.EXEC ]${NC} ${YELLOW}進入自定義架構師調優模式...${NC}"
    check_existing || return
    ...
    echo -e "${CYAN}[ SYS.CONF ]${NC} 路由協議開關 (NAT 環境建議按需啓用以節約埠)"
    ...
    echo -e "\n${CYAN}[ SYS.NET  ]${NC} 物理層埠分配"
    if [ "$ENABLE_VD" == "1" ]; then
        reading "VLESS (WS) 監聽埠 (直接回車由系統隨機分配)" PORT_VD
        [ -z "$PORT_VD" ] && while true; do PORT_VD=$((RANDOM % 50000 + 10000)); check_port_usage $PORT_VD && break; done
    fi
    if [ "$ENABLE_RE" == "1" ]; then
        reading "Reality 監聽埠 (強烈建議配置443，直接回車隨機分配)" PORT_RE
    ...
    echo ""; cyber_progress "向內存注入架構配置並喚醒守護進程..."
    generate_config; setup_services
...
manage_protocols() {
    ...
    while true; do
        ...
        echo -e "${CYAN} ||----------------------------------------------------------------|| ${NC}"
        [ "$ENABLE_VD" == "1" ] && echo -e "${CYAN} ||${NC}  [1] [ MOD ] 修改 VLESS (WS) 參數   ${YELLOW}(當前埠: $PORT_VD)${NC}"
        [ "$ENABLE_RE" == "1" ] && echo -e "${CYAN} ||${NC}  [2] [ MOD ] 修改 Reality 參數      ${YELLOW}(當前埠: $PORT_RE)${NC}"
        [ "$ENABLE_HY" == "1" ] && echo -e "${CYAN} ||${NC}  [3] [ MOD ] 修改 Hy2 參數          ${YELLOW}(當前埠: $PORT_HY)${NC}"
        [ "$ENABLE_TC" == "1" ] && echo -e "${CYAN} ||${NC}  [4] [ MOD ] 修改 TUIC v5 參數      ${YELLOW}(當前埠: $PORT_TC)${NC}"
        [ "$ENABLE_S5" == "1" ] && echo -e "${CYAN} ||${NC}  [5] [ MOD ] 修改 SOCKS5 參數       ${YELLOW}(當前埠: $PORT_S5)${NC}"
        [ "$ENABLE_ARGO" == "1" ] && echo -e "${CYAN} ||${NC}  [6] [ MOD ] 調整 Argo 隧道配置     ${YELLOW}(當前模式: $ARGO_MODE)${NC}"
        echo -e "${CYAN} ||----------------------------------------------------------------|| ${NC}"
        echo -e "${CYAN} ||${NC}  ${RED}[7] [ KILL ] 強制停用/卸載指定協議釋放內存與埠${NC}"
        ...
        case $choice in
            ...
            2)
                ...
                reading "輸入新 Reality 埠 (直接回車保持當前不變)" p; [ -n "$p" ] && PORT_RE=$p
                reading "替換偽裝 SNI 域名 (當前探測目標: $REALITY_SNI)" s; [ -n "$s" ] && REALITY_SNI=$s
                ;;
            3) [ "$ENABLE_HY" != "1" ] && continue; reading "輸入新 Hy2 埠 (直接回車保持不變)" p; [ -n "$p" ] && PORT_HY=$p; reading "替換握手密碼 (直接回車保持不變)" pw; [ -n "$pw" ] && PW_HY=$pw ;;
            4) [ "$ENABLE_TC" != "1" ] && continue; reading "輸入新 TUIC 埠 (直接回車保持不變)" p; [ -n "$p" ] && PORT_TC=$p; reading "替換握手密碼 (直接回車保持不變)" pw; [ -n "$pw" ] && PW_TC=$pw ;;
            5) [ "$ENABLE_S5" != "1" ] && continue; reading "輸入新 Socks5 埠 (直接回車保持不變)" p; [ -n "$p" ] && PORT_S5=$p; reading "替換認證密碼 (直接回車保持不變)" pw; [ -n "$pw" ] && S5_P=$pw ;;
            6)
...
show_nodes() {
    ...
    if [ "$ENABLE_VD" == "1" ]; then
        if [ "$VD_MODE" == "1" ]; then
            ...
            echo -e "${CYAN} ||${NC}  ${GREEN}[ VLESS + WS + TLS ]${NC} (掛載內核自簽名偽裝證書)"
